...
// Carousel with smoother animations
window.addEventListener('DOMContentLoaded', function() {
    ...
    function updateSlides(direction) {
      ...
      slide.forEach((elem) => {
        ...
        elem.classList.remove('slide-jellyfish', 'slide-jellyfish-reverse');
        
        // Remove positioning classes
        if (elem !== activeSlide && elem !== preactiveSlide && elem !== proactiveSlide) {
      ...
      // Add the jellyfish animation to the active slide
      if (direction === 'next') {
        activeSlide.classList.add('slide-jellyfish');
      } else {
        activeSlide.classList.add('slide-jellyfish-reverse');
      }
    ...
    // Navigation Functions - Fixed direction (left button moves to next, right button moves to prev)
    window.nextProject = function() {
      // Left button (next project)
      const nextBtn = document.getElementById('prev-project'); // Using prev-project ID for next function
      if (nextBtn) {
        nextBtn.classList.add('bouncy-button');
        setTimeout(() => nextBtn.classList.remove('bouncy-button'), 800);
      }
    ...
    window.prevProject = function() {
      // Right button (previous project)
      const prevBtn = document.getElementById('next-project'); // Using next-project ID for prev function
      if (prevBtn) {
        prevBtn.classList.add('bouncy-button');
        setTimeout(() => prevBtn.classList.remove('bouncy-button'), 800);
      }
    ...
    // Bind event listeners - Fixed button directions
    const nextBtn = document.getElementById('prev-project'); // Left button for next
    const prevBtn = document.getElementById('next-project'); // Right button for prev
