...
// recent.json is generated by scripts/sync-vault.sh - most recently
// modified vault notes, newest first
const recentModules = import.meta.glob('../../../obsidian/recent.json', {
  query: '?raw',
  import: 'default',
  eager: true,
});

/** Paths of recently worked-on notes, newest first ([] if no manifest). */
export function getRecentPaths(): string[] {
  try {
    const raw = Object.values(recentModules)[0] as string | undefined;
...
/** When the vault last synced (ISO string), if the manifest records it. */
export function getLastSynced(): string | null {
  try {
    const raw = Object.values(recentModules)[0] as string | undefined;
    const parsed = raw ? JSON.parse(raw) : null;
    return parsed && !Array.isArray(parsed) && parsed.generated ? parsed.generated : null;
  } catch {
    return null;
  }
}

/** Posts ordered by "recently worked on" (falls back to natural order). */
export async function fetchRecentPosts(limit = 2): Promise<BlogPost[]> {
  const posts = await fetchMarkdownFiles();
  const recent = getRecentPaths();
  if (!recent.length) return posts.slice(0, limit);
  const rank = new Map(recent.map((p, i) => [p, i]));
  const ranked = posts
    .filter((p) => rank.has(p.path))
    .sort((a, b) => rank.get(a.path)! - rank.get(b.path)!);
  // top up with unranked posts if the manifest covers fewer than `limit`
  for (const p of posts) {
    if (ranked.length >= limit) break;
    if (!rank.has(p.path)) ranked.push(p);
  }
