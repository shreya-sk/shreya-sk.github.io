...
**CMD** — the default command. Completely replaced if you pass anything after the image name:

```bash
docker run myapp                  # runs: python app.py  (CMD default)
docker run myapp python debug.py  # runs: python debug.py  (CMD replaced)
```

**ENTRYPOINT** — the fixed executable. Always runs. Arguments from `docker run` are appended to it, not replacing it:
...
docker run myapp debug.py  # runs: python debug.py
```

**Used together** — ENTRYPOINT is the executable, CMD is the default argument:
...
```bash
docker run myapp              # runs: python app.py  (CMD is the default arg)
docker run myapp debug.py     # runs: python debug.py  (CMD overridden)
```

| | Replaceable by `docker run` args? | Use for |
|---|---|---|
| `CMD` | Yes — entirely | Default command when nothing is specified |
| `ENTRYPOINT` | No (only with `--entrypoint` flag) | The core executable that always runs |

**Rule of thumb:** use ENTRYPOINT for the thing that always runs, CMD for the default argument you're likely to swap.
...
- See [[Docker Workflow]] for where Docker Hub fits in the full build → deploy pipeline
...
- Docker isolates all content, code, and data in a container from your local filesystem. Sometimes you may want the container to access a directory on your system. This is when you use bind mounts. See [[Docker Commands#`-v` (volume mount)|-v in Docker Commands]] for how this works without Compose.
