...
__export(main_exports, {
  ExplainSelectionWithAiModal: () => ExplainSelectionWithAiModal,
  default: () => ExplainSelectionWithAiPlugin
});
...
var ExplainSelectionWithAiPlugin = class extends import_obsidian.Plugin {
  async onload() {
    ...
    this.registerEvent(
      this.app.workspace.on("editor-menu", (menu, editor, view) => {
        menu.addItem((item) => {
          ...
          const label = selection.length > 24 ? selection.substring(0, 24) + "..." : selection;
          item.setTitle(`Expand on "${label}" in context with AI`).setIcon("document").onClick(async () => {
            const selection2 = editor.getSelection();
            const cursor = editor.getCursor();
            const lineText = editor.getLine(cursor.line).substring(
...
var ExplainSelectionWithAiModal = class extends import_obsidian.Modal {
  constructor(app, openai, endpoint, userSelection, selectionContext, plugin) {
    super(app);
    this.app = app;
    this.plugin = plugin;
    ...
    try {
      ...
      for await (const chunk of completion) {
        if (chunk.choices[0].delta.content) {
          ...
          contentBox.empty();
          import_obsidian.MarkdownRenderer.render(this.app, rollingText, contentBox, "/", this.plugin);
        }
      }
    } catch (err) {
...
var ExplainSelectionWithAiSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("LLM provider").setDesc(
      ...
      (dropdown) => {
        dropdown.addOption("openai", "OpenAI (remote)").addOption("ollama", "Ollama (local)").addOption("custom", "Custom").setValue(this.plugin.settings.dropdownValue).onChange(async (value) => {
  ...
  displayConditionalSettings(containerEl) {
    ...
    if (this.plugin.settings.dropdownValue === "openai") {
      new import_obsidian.Setting(containerEl).setName("OpenAI model").setDesc("Select the OpenAI model you want to use.").addDropdown((dropdown) => {
        dropdown.addOption("gpt-3.5-turbo", "gpt-3.5-turbo").addOption("gpt-4o", "gpt-4o").addOption("gpt-4-turbo", "gpt-4-turbo").setValue(this.plugin.settings.endpoint).onChange(async (value) => {
          this.plugin.settings.endpoint = value;
          await this.plugin.saveSettings();
        });
      }).setClass("conditional-setting");
      new import_obsidian.Setting(containerEl).setName("API Key").setDesc("Enter your OpenAI API key. (required)").addText((text) => {
      ...
      new import_obsidian.Setting(containerEl).setName("Selected Ollama model").setDesc("Choose the Ollama model you want to use. Make sure you have Ollama installed and running with the selected model pulled.").addDropdown((dropdown) => {
        dropdown.addOption("llama3", "llama3").addOption("mistral", "mistral").setValue(this.plugin.settings.endpoint).onChange(async (value) => {
          this.plugin.settings.endpoint = value;
          await this.plugin.saveSettings();
        });
      }).setClass("conditional-setting");
    } else if (this.plugin.settings.dropdownValue === "custom") {
      new import_obsidian.Setting(containerEl).setName("Base URL").setDesc("Enter your custom base URL.").addText((text) => {
        text.setValue(this.plugin.settings.baseURL).onChange(async (value) => {
          this.plugin.settings.baseURL = value;
          await this.plugin.saveSettings();
        });
      }).setClass("conditional-setting");
      new import_obsidian.Setting(containerEl).setName("Endpoint").setDesc("Enter your custom endpoint.").addText((text) => {
        text.setValue(this.plugin.settings.endpoint).onChange(async (value) => {
          this.plugin.settings.endpoint = value;
          await this.plugin.saveSettings();
        });
      }).setClass("conditional-setting");
      new import_obsidian.Setting(containerEl).setName("API Key").setDesc("Enter your custom API key. (Optional)").addText((text) => {
        text.setValue(this.plugin.settings.apiKey).onChange(async (value) => {
          this.plugin.settings.apiKey = value;
          await this.plugin.saveSettings();
        });
